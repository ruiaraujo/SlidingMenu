...
public class CustomViewAbove extends ViewGroup {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_DOWN && BuildConfig.DEBUG)
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (BuildConfig.DEBUG) Log.v(TAG, "onInterceptTouch moved to:(" + x + ", " + y + "), diff:(" + xDiff + ", " + yDiff + "), mLastMotionX:" + mLastMotionX);
			if (xDiff > mTouchSlop && xDiff > yDiff && thisSlideAllowed(dx)) {
				if (BuildConfig.DEBUG) Log.v(TAG, "Starting drag! from onInterceptTouch");
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (!mIsBeingDragged) {
				...
				final float y = MotionEventCompat.getY(ev, pointerIndex);
				final float yDiff = Math.abs(y - mLastMotionY);
				if (BuildConfig.DEBUG) Log.v(TAG, "onTouch moved to:(" + x + ", " + y + "), diff:(" + xDiff + ", " + yDiff + "), mLastMotionX:" + mLastMotionX);
				if (xDiff > mTouchSlop && xDiff > yDiff && thisSlideAllowed(dx)) {
					if (BuildConfig.DEBUG) Log.v(TAG, "Starting drag! from onTouch");
					mIsBeingDragged = true;
					mLastMotionX = x;
					setScrollingCacheEnabled(true);
				}
...
//			invalidate();
	...
	private void onSecondaryPointerUp(MotionEvent ev) {
		if (BuildConfig.DEBUG) Log.v(TAG, "onSecondaryPointerUp called");
